...
for d = 10:10:50
    for j = 0:(repetitions-1)        
        filenamesCbrCell(end+1) = {strcat('../../results/d2dVsCell/', 'd', int2str(d), '_rep', int2str(j), '.sca.cell')};   
        filenamesCbrD2D(end+1) = {strcat('../../results/d2dVsCell/', 'd', int2str(d), '_rep', int2str(j), '.sca.d2d')};           
    end        
end
