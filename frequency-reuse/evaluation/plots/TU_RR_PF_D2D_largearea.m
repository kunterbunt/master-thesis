...
for n=nMin:nMax    
    for rep=0:(reps - 1)        
        filenamesRR(j, (rep+1)) = {strcat('../../results/RR-D2D-largearea/', 'n', int2str(n), '_rep', int2str(rep), '.sca.voip')};           
        filenamesPF(j, (rep+1)) = {strcat('../../results/PF-D2D-largearea/', 'n', int2str(n), '_rep', int2str(rep), '.sca.voip')};           
        filenamesTU(j, (rep+1)) = {strcat('../../results/TU-D2D-largearea/', 'n', int2str(n), '_rep', int2str(rep), '.sca.voip')};           
        
        filenamesRRDelay(j, (rep+1)) = {strcat('../../results/RR-D2D-largearea/', 'n', int2str(n), '_rep', int2str(rep), '.sca.delay')};           
        filenamesPFDelay(j, (rep+1)) = {strcat('../../results/PF-D2D-largearea/', 'n', int2str(n), '_rep', int2str(rep), '.sca.delay')};           
        filenamesTUDelay(j, (rep+1)) = {strcat('../../results/TU-D2D-largearea/', 'n', int2str(n), '_rep', int2str(rep), '.sca.delay')};           
    end
    j = j + 1;
end
